...
export default function Dashboard() {
  ...
  useEffect(() => {
    document.documentElement.dataset.theme = theme; // hook into CSS variables
  }, [theme]);
  ...
  const addTask = (data: TaskFormData) => {
    ...
    setTasks(prev => [
      { id: crypto.randomUUID(), title: data.title, description: data.description, status: data.status, priority: data.priority, createdAt: now, dueDate: data.dueDate, updatedAt: now },
      ...prev,
    ]);
  };
  ...
  const toggleStatus = (id: string) =>
    setTasks(prev =>
      prev.map(t =>
        t.id === id
          ? { ...t, status: t.status === 'done' ? 'todo' : t.status === 'todo' ? 'in-progress' : 'done', updatedAt: new Date().toISOString() }
          : t
      )
    );
    ...
    <div className="dashboard">
      <header className="dashboard__header">
        ...
        <div className="header__actions">
          ...
          <label>
            Import
            <input type="file" accept="application/json" onChange={e => e.target.files && importData(e.target.files[0])} style={{ display: 'none' }} />
          </label>
        </div>
        <div className="stats">
